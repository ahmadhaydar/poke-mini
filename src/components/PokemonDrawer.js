...
export const PokemonDrawer = ({ onClose, isOpen, pokemonData }) => {
  return <Drawer size="md" placement="right" onClose={onClose} isOpen={isOpen}>
    <DrawerOverlay />
    <DrawerContent>
      <Flex
        direction="column"
              ...
              <TabPanel>
                {pokemonData?.stats.map(({ base_stat, stat }) => (
                  <HStack mb="4" width="100%" spacing="0">
                    <Text
                      width="30%"
                      textTransform={stat.name === "hp" ? "uppercase" : "capitalize"}
                      color="gray.500"
                    >
                      {stat.name}
                    </Text>
